...
float cartes[XYZ] = { 0 };
  ...
  bool man_disable_filament_ran_out = true;
...
void prepare_move_to_destination() {
  ...
  void handle_filament_runout() {
    if ( settings.load_file_runout_state() && (!filament_ran_out)) {
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
  ...
  #if ENABLED(FILAMENT_RUNOUT_SENSOR)
    if ((IS_SD_PRINTING || print_job_timer.isRunning()) && (settings.load_file_runout_state()) && (READ(FIL_RUNOUT_PIN) == FIL_RUNOUT_INVERTING))
...
#ifdef PXMALION_DPRINTER
extern void user_config_manage(void);
